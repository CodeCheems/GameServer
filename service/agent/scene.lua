...
s.client.enter = function(msg)
    ...
    local snameAddr = skynet.localname("."..sname)
    local isok = s.call(snode,snameAddr,"enter",s.id,mynode,skynet.self())
    if not isok then
        return {"enter",2,"进入失败"}
    end
    s.snode = snode
...
s.client.shift = function(msg)
    ...
    local x = msg[2] or 0
    local y = msg[3] or 0
    local snameAddr = skynet.localname("."..s.sname)
    s.call(s.snode,snameAddr,"shift",s.id,x,y)
end
